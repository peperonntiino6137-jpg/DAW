...
## LUFS メータリング（ITU-R BS.1770-4）と書き出しラウドネスレポート（`js/loudness.js`・2026-08-20）

METERING タブ（index.html に枠だけあったもの）を実装した。Momentary(400ms) / Short Term(3s) /
Integrated（リセットボタン付き）/ True Peak を表示し、exportMix はレポート
（Integrated LUFS / True Peak dBTP）を残す。

- **K特性フィルタの係数はサンプルレートから毎回計算する**。仕様書の係数表は 48kHz 専用なので、
  プリワープしたアナログ原型（シェルフ f0=1681.97Hz/G=+4dB/Q=0.7072、ハイパス f0=38.14Hz/Q=0.5003）
  から双一次変換で求める。48kHz で仕様書 Table 1/2 と**最大誤差 9e-16** で一致（テストで照合）。
  44.1kHz の 997Hz 正弦波でも -17.997 LUFS と、レートに依らず ±0.01 に収まる。
- **ゲーティング**: 100ms ホップの自乗和を蓄積し、4ホップ = 400ms ブロック（75% 重なり）。
  絶対ゲート -70 LUFS は蓄積時、相対ゲート -10 LU は integrated() を呼ぶたびにかけ直す
  （相対しきい値は全ブロックの平均に依存するので、後からしか決められない）。
- **True Peak は 4倍ポリフェーズ FIR**（窓付き sinc 48タップ、位相ごとに DC 正規化）。
  fs/4 を 45° 位相で切った「サンプル点に乗らないピーク」を -6.09dBTP と、真値 -6.02 の
  0.07dB 差で検出できる。
- **ライブ計測は AudioWorklet（data: URL、リミッターと同じ方式）を選んだ**。AnalyserNode の
  時系列読み（rAF 30fps）は窓が読み出しタイミングに依存して**取りこぼしと二重読みが起きる**ため、
  Momentary はともかく Integrated（全期間の蓄積）が不正確になる。Worklet なら欠落なしの
  サンプル列で計測でき、負荷は 100ms ホップ完了ごとに 10Hz で postMessage するだけ。
  メインスレッド（objui）は `DAW.loudness.live` を既存の 30fps メーター更新で読むだけで計算しない。
  タップは **masterGain 直後 = リミッター前**（ピークメーターと同じ規約）。METERING タブを
  一度開いたときだけノードを作る（開かなければ負荷ゼロ）。閉じても計測は続ける
  （Integrated は聴取全体の蓄積が意味を持つため）。
- **DSP 本体は coreSource() の1か所だけ**（リミッターと同じ構図）。Worklet も、書き出しレポートや
  テストが使う純関数 `DAW.loudness.integrated(buffers, sr)` / `truePeak()` /
  逐次計算器 `createMeter()` も、同じソースから生成する。
- **落とし穴: process() はサンプル主導で回す**。チャンネル主導（ch0 を全部 → ch1 を全部）だと、
  ホップ境界がフレーム途中に落ちたとき他チャンネルの「未来のサンプル」が現在のホップに混ざる。
  オフライン一括処理（frames=全長）では完全に壊れる。
- **書き出しレポートは alert にしない**。既存の規約「書き出しでは alert を出さない」をテストが
  固定している（tests.js [8]）ため、ヒントバー（setHint）へ流し、METERING タブの
  「書き出しレポート」欄にも残す（`DAW.wav.lastExportLoudness`）。0dBFS confirm の相当物として
  「完了時に必ず見える表示」はヒントバーが担う。
- **チャンネル重み**: 2ch = 1/1、5.1 = L/R/C 1.0・LFE 除外・Ls/Rs 1.41（書き出しの多チャンネル
  レポートにも適用）。7.1.4 は 90° のサイドのみ 1.41（135° 後方と上層は仕様の 60〜120° 水平層に
  入らないので 1.0）。

### 検証（`test/tests-loudness.js`・グループ [43]・30件）

997Hz -18dBFS ステレオ = **-18.000 LUFS**（期待値の根拠: 各ch平均自乗 a²/2 の2ch和 = a² = -18dB、
K特性の 997Hz ゲイン +0.691dB が定数 -0.691 と相殺。EBU tech 3341 テスト1の -18dBFS 版）/
48kHz 係数表照合 / 44.1kHz / 逐次計算器の M・S・I と reset / 無音混在でも Integrated 差 0.033 LU /
インターサンプルピーク検出 / exportMix レポート実経路（-18.007 LUFS）/ METERING タブの表示・
リセット / ライブ AudioWorklet 経路（再生 1 秒で M=-9.0 LUFS ≒ 理論値 -9.03）。
既存の [28] V.52 は「METERING タブは disabled」を固定していたため実装済み前提に更新。
全 1233 件パス（+31）。

## スピーカーレイアウト拡充（2.0 / 5.0.5.3=13ch）+ VBAP N層一般化、ピーク発光と ch 別出力メーター（2026-08-20）

改善ロードマップ項目⑦（レイアウト拡充）と①（ピーク連動発光 + ch 別メーター）を実装した。

### VBAP の N層一般化（`js/objaudio.js`）

従来の vbapGains は「水平層 + 上層（el>=25）」の2層固定で、el<0 は t=clamp(el/elUp,0,1) により
0 へクランプされて下層スピーカーを鳴らせなかった。次の形へ一般化した:

- **層分割 `vbapLayers(layout)`**: LFE を除き仰角順に並べ、仰角差 10° 未満の連なりを同じ層に
  まとめる（5.1=1層、7.1.4=2層 0°/45°、5.0.5.3=3層 -20°/0°/+30°）。層の代表仰角は所属
  スピーカーの平均。結果は WeakMap でキャッシュ（bakePath が時刻列ぶん呼ぶため）。
- **層間は el を挟む隣接2層の等パワークロスフェード**（cos/sin(t·π/2)、t は代表仰角の線形比）。
  範囲外の el は端の層へクランプ（1層の 5.1 は従来どおり水平へ落ちる）。層内は従来の
  layerGains（ペア VBAP）のまま。パワー正規化も従来のまま。
- **回帰なしの確認**: 旧実装を node で再現し、5.1/7.1.4 の全域（az 5° × el 5° 刻み）で
  新旧の最大差 6.1e-17（浮動小数の丸めのみ）。7.1.4 の旧式 t=el/45 と新式 t=(el-0)/(45-0) が
  厳密に同値になるため。テストでも数値固定（[45] N.25〜N.30、既存 [30] は無変更で全パス）。

### レイアウト定義（LAYOUTS が正、二重定義禁止のまま）

- **2.0**: L(+30°)/R(-30°)。1層なのでペア VBAP がそのまま等パワーパンになる（正面で各 0.7071）。
- **5.0.5.3（13ch、WalkMix リファレンス互換）**: LFE なし。チャンネル順 = 耳高 → 上層 → 下層:
  - 耳高 5本 (el 0°): L(+30) R(-30) C(0) Ls(+110) Rs(-110)
  - 上層 5本 (el +30°): Lt(+30) Rt(-30) Ct(0) Lst(+110) Rst(-110)
  - 下層 3本 (el -20°): Lb(+30) Rb(-30) Cb(0)
- RENDERER の配置ボタン・スピーカー表・3D球の番号・書き出し ch 数はすべて LAYOUTS から
  導出済みなので自動追従（テストで固定）。ラウドネスの `weightsFor` は 6/12ch の従来表を
  維持しつつ、それ以外は LAYOUTS から BS.1770 の規則（LFE=0、耳高 |az|60〜120°=1.41）で
  導出するようにした（13ch は Ls/Rs だけ 1.41）。

### ピーク連動発光 + ch 別出力メーター（`js/objui.js` / `js/audio.js`）

- **発光**: 3ビュー（TOP/FRONT/3D球）の点描画に `glowOf(objId)` を加えた。値は updateMeters が
  30fps で貯める既存の peaks（線形）を読むだけで、描画のたびに解析しない。-60dB→0 / 0dB→1 の
  線形マップで、加算合成（globalCompositeOperation='lighter'）のラジアルグラデーションのハロ +
  点の明るさ加算。**再生中のみ**（停止中は従来の見た目と同一）。tick は再生中、経路オブジェクトが
  あれば毎フレーム、無ければメーター更新（30fps）に合わせて canvas を描き直す。
  停止中の計測は従来どおり可視ストリップのみ、再生中だけ発光用に全オブジェクトを読む
  （30fps 間引きの中なので 128 個でも 512 サンプル × 128 × 30/s 程度で軽い）。
- **ch 別出力メーター**: RENDERER に「出力メーター」セクションを追加。本数は
  `chMeterNames()`（スピーカー出力時は配置の ch 数 = WAV のチャンネル順、バイノーラル時は L/R）
  から可変生成するので、レイアウトを増やしてもここは変えなくてよい（13ch にそのまま追従）。
  タップは `DAW.audio.getChannelLevels(n)` = **masterGain 直後 = リミッター前**（既存規約）。
  ChannelSplitter(n) は discrete 解釈なので、2ch 再生中に本数を広げても嘘のレベルは出ない。
  本数が変わったら旧タップを disconnect して組み直す。RENDERER が隠れている間は読まない。

### 検証（`test/tests-vbap13.js`・グループ [45] 39件 / [46] 21件）

[45]: 2.0/13ch の定義・層分割・既知方位のゲイン分配（各層の真上・中点で 0.7071 等の数値固定）・
全域パワー和≈1・下層 -20° が実際に鳴る（オフライン書き出しの 11ch 目 RMS 0.3535）・
5.1/7.1.4 の回帰（分配値を 1e-9 で固定）・2ch/13ch の書き出し ch 数・weightsFor。
[46]: メーター列の本数とラベルの追従（6→13→2）・ライブ再生の ch 分配
（az=+30 の 0.5 正弦波で L メーター 0.500、他 0.000 / 13ch 下層 Lb 0.500）・発光マップの数値固定・
再生中のみ発光・3ビュー描画へのハロ伝搬・停止中は可視ストリップのみ計測の回帰。
既存 [32] W.47 は「配置ボタンは2つ」を固定していたため LAYOUTS 全数に更新。全 1389 件パス（+62）。

## 軌道ジェネレータと経路ループ（`js/objects.js` / `js/objaudio.js` / `js/adm.js` / `js/objui.js`・2026-08-20）

改善ロードマップ⑤。経路（位置オートメーション）の waypoint 列を自動生成する
`DAW.objects.generatePath(id, kind, opts)` と、経路を周回させる `obj.path.loop` を追加した。

### 設計

- **生成は「普通の waypoint 列を吐くだけ」**。専用のデータ形式は持たず、既存の点を
  `loadPath` と同じ検証（normalize / clamp / 昇順 / PATH_MIN_DT）を通して置き換える。
  これで編集・undo（history.commit 1回で実行前に戻る）・書き出しの焼き込み（bakePath）・
  ADM 書き出し・保存が**無改修で全部効く**。形状は4種:
  - `circle`   一定 el のまま az を一周（`dir` で回転方向）
  - `spiral`   az を回しながら el（と任意で dist）を始点→終点へ漸変
  - `eight`    az が中心 ±radius、el が倍周波数で上下するリサージュ 1:2 の8の字
  - `pingpong` az 中心 −radius ↔ +radius の2点間往復（直線往復に中間点は不要なので折り返しにだけ点を置く）
  - opts は `t0` / `period`（`beats` 指定は BPM から換算して優先）/ `cycles` / `div`
    （1周期の点数。既定 8 = 1/8 周期ごと・ease 'linear'。円滑さは既存のセグメント補間に任せる）/
    中心 `az/el/dist`（既定は現在位置）/ `radius`（1〜90°。90° 超は pingpong の2点が
    180° を超えて離れ、azLerp の最短弧が裏へ回ってしまうため）/ `el1` / `dist1` / `dir`。
- **ループとホールドの整合（設計判断）**: `pathPosAt` のホールド仕様は
  「最初の点より前 = 先頭でホールド」「最後の点より後 = 末尾でホールド」だった。
  loop 有効時は後者だけを置き換え、**t を経路スパン（最後の点 − 最初の点）で剰余して
  先頭へ巻き戻す**（`t' = t0 + ((t−t0) mod span)`。剰余は必ず [0, span) に落とし、
  JS の `%` が返す負の剰余を出さない）。前者（開始前のホールド）は loop でも従来のまま
  残す — 「経路が始まる前」はループの位相が定義できず、そこまで周回で埋めると
  再生開始位置や経路の平行移動で音が変わる非決定性を持ち込むため。折り返しの瞬間
  t = 末尾 + k·span は剰余 0 = 先頭の点そのもの。**時間の繋ぎは剰余・空間の繋ぎは
  経路の閉じ方に従う**という分担にした: ジェネレータは終端に始点と同値の「閉じ点」を
  置くので途切れなく周回し、手で組んだ開いた経路では端から端へ瞬間移動になる。
  折り返しに専用の遷移時間を勝手に挿入して繋ぐ案は、周期が span からずれて拍と
  合わなくなるため退けた（瞬間移動はライブは setTargetAtTime の 〜20ms グライド、
  書き出しは折り返し境界のイベントで同様に一瞬で追従する。トランスポートループの
  折り返しと同じ扱い）。1点だけ・スパン 0 の経路は loop でも従来どおりホールド
  （0 除算を踏まない）。
- **書き出しの一致**: `bakeTimes` に loop を渡すと、刻みを最後の点で止めず until まで
  続け、waypoint 時刻も周回ごとに展開する（折り返し境界 = 先頭点の時刻も各周回に入る）。
  位置の値は従来どおり全サンプル時刻で `pathPosAt` を引くだけなので、剰余の実装が
  1箇所（モデル）に閉じたまま、焼き込んだランプ・ADM の audioBlockFormat・ライブ追従
  （followPaths）が同じ値で一致する。
- **UI は最小限**: 経路ツールに「生成」ボタン → 既存 `ui.showMenu` で形状4種+ループ切替、
  形状を選ぶとメニュー直下に使い捨てのパラメータ枠（周期 / 回数 / 形状固有の角度 +
  生成ボタン）。開始時刻は再生ヘッド位置。生成 + 経路の自動有効化をまとめて
  history.commit 1回 = undo 1回で実行前に戻る。ヒントバー連動。
  loop は toJSON / loadPath / stateSig（enabled と同じワードにビット追加）に混ぜた。

### 検証（`test/tests-objgen.js`・グループ [45]・82件）

各形状の waypoint 座標列を数値固定（circle の 45° 刻みと ±180 折り畳み / spiral の
el・dist 漸変 / eight のリサージュ値 / pingpong の折り返し点のみ）/ beats 換算 /
周期・振幅のクランプ / pathPosAt の剰余（スパン跨ぎ・10万秒先でも負にならない・
1点はホールド）/ bakeTimes のループ展開（周回ごとの waypoint・from が周回途中の場合）/
ループ有効の書き出し WAV で 1周目と2周目の同位相窓が同音量・L/R 比が posAt の
ゲイン比と一致 / ADM ブロックの周回展開 / 保存往復（旧データは loop=false）/
生成・ループ切替とも undo 1回 / lock 拒否（モデル・UI 両方）。全 1409 件パス（+82）。

## オブジェクトのグループ化（`js/objects.js` / `js/objaudio.js` / `js/objui.js` / `js/history.js` / `js/wav.js`・2026-08-20）

改善ロードマップ⑧（最終項目）・第1段。複数オブジェクトを束ねて「まとめて回す」。

### データ設計

- **所属は `obj.groupId`（追加フィールドのみ = 後方互換）**。グループ本体は
  `DAW.objects.groups` のレジストリ（id / name / color だけ）で、位置・音量などの状態は
  持たない —— 状態は各メンバーが持ち続けるので、保存・undo・書き出し・ADM は
  メンバー単位のまま**無改修で効く**。
- **保存は別キー `objectGroups`**（`groupsToJSON` / `loadGroups`）。`objects` を
  `{list, groups}` に変える案は退けた: 既存テスト（O.34）も旧実装の load も
  「objects は配列」を前提にしており、配列のまま + 追加キーなら**旧実装でも
  グループ抜きでそのまま開ける**（version:1 のまま互換）。履歴スナップショット
  （history.snapshot / apply）にも同じキーで入れた。
- **整合は loadGroups が一手に取る**（必ず `load(objects)` の後に呼ぶ規約）:
  groupId がレジストリに無いグループを指す（手で編集されたファイル）→ 既定の
  名前・色で作って補完。メンバーのいないグループ → 畳む（pruneGroups。所属変更・
  オブジェクト削除の後も同じで、メニューに空グループを残さない）。
- `setGroup` は lock を見ない（assignTrack と同じ理由: グルーピングは編成の操作で、
  lock='all' から外せないと編成が永久に固まる）。

### グループ回転の数学

- **どの操作も「全員へ同じ1つの回転」をかける**。回転は球面の等長変換なので、
  メンバー間の球面距離（相対配置）は厳密に保存され、グループ中心
  （`groupCenter` = toCartesian 平均→正規化）も同じ回転で付いてくる。
  dist は各メンバーが自分の値を維持する（回転は方向だけを変える）。
- **トップビュー = ヨー回転（`rotateGroupAz`）**: 掴んだメンバーの az 差分を全員の az へ
  足すだけ（鉛直軸まわりの回転そのもの）。最小回転をここでも使う案は退けた:
  el≠0 のメンバーを真上から見た図でドラッグすると回転軸が傾き、他メンバーの
  仰角が「勝手に」動いて見える。真上からの図では el 不変のヨーが予測どおりに動く。
- **正面/3D球ビュー = 最小回転（`rotateGroupTo`）**: 掴んだ点の現在方向 u → ポインタ方向 v
  への最小回転（軸 = u×v、角 = ∠uv）をロドリゲスの公式で全員へ。最小回転は
  「v 軸まわりの余計なねじれ」を持たない一意な回転で、掴んだ点はポインタへ正確に
  追従する。u ≒ −v の縮退（正反対へ一気に跨ぐ）は u に直交する軸での 180° 回転に
  落とす保険を入れた（ドラッグは連続に動くので実質踏まないが、API 直呼びで壊れない）。
- 極座標⇔直交の変換は `DAW.objaudio.toCartesian` / **新設の `fromCartesian`** に一本化
  （objui.fromCartesian は委譲に変更。式の複製 = 鏡像事故の轍を増やさない）。
- **回転しないメンバー**: lock（canEditPosition=false）と経路有効（位置は path が
  駆動中。経路ごと回すのは第2段以降の課題）。掴んだメンバー自身が動かせない場合は
  回転そのものを不成立にする（false）。

### UI（Shift+ドラッグの選定理由）

- **通常ドラッグ = 従来どおり単独移動、Shift+ドラッグ = グループごと回転**。
  「グループ所属なら常にまとめて動く」案は、1個だけ直したいときに一時解除が要る
  誤爆持ちで退けた。Shift は既存操作（クリップのループ区間・ノブ微調整）と同じ
  修飾キーの語彙で学習コストが低く、押していなければ何も変わらない。
  判定はドラッグ開始時に固定（途中で Shift を離しても回転のまま = 1ジェスチャ1意味）。
- ストリップ: 最上部にグループ色帯（.os-group。無所属は非表示）、**右クリック →
  既存 ui.showMenu** で「既存グループへ追加（メンバー数付き・所属中は ✓ で無効）/
  新しいグループを作って追加 / グループから外す」。3ビューでは選択オブジェクトと
  同じグループのメンバーに細枠（drawGroupRing。選択リングと同色・細め）。
  ヒントバー連動（メニュー時と回転開始時）。色帯の tooltip に Shift+ドラッグと
  除外条件（固定中・経路有効）を明示。
- **undo 粒度は既存規約のまま**: ドラッグ1回 = pointerup で commit 1回、メニュー操作
  1回 = commit 1回（新規作成は作成+追加で1回）。
- stateSig に `o.groupId` とグループレジストリ（id/name/color）を混入（既知の罠。
  混ぜ忘れると undo 後に色帯・細枠が追従しない）。

### 第1段で見送った項目（第2段の候補）

- **Gain 一括（相対 dB シフト）/ グループ mute・solo / グループ Width**: モデルに足すのは
  数行だが、対応する UI（ストリップの一括操作の見せ方）を含めると別設計が要るため
  見送り。回転（空間操作）だけを先に出し、ミキシング系の一括操作はまとめて次段で。
- グループの改名・色変更 UI（レジストリは name/color を保存・undo 対応済み。UI が未着手）。
- 経路有効メンバーの「経路ごと回転」（waypoint 列への回転適用）。

### 検証（`test/tests-objgroup.js`・グループ [47]・58件）

モデル 37件: レジストリ CRUD と空グループの自動整理 / ヨー回転・最小回転の既知座標
数値固定（(0,0)→(0,30) で軸上の (90,0) が不動、赤道上はヨーと一致、一般回転・正反対
縮退でも球面相対を厳密保持）/ el・dist 維持 / lock メンバー不動・経路有効メンバー除外・
掴んだ本人が lock なら不成立 / groupCenter（45° の既知値・縮退 null）/ 保存往復
（objects は配列のまま + objectGroups）/ 旧プロジェクト・レジストリ欠落・空グループの
補完 / undo・redo。UI 21件: 右クリックメニューの追加・✓・外す / 色帯の表示・非表示 /
Shift+ドラッグの回転（トップ = ヨー、正面 = 最小回転）と dist 維持 / ドラッグ1回 =
undo 1エントリ / 通常ドラッグは単独移動のまま / lock メンバー不動 / stateSig 変化。
全 1531 件パス（+60）。
